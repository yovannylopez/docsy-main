...
func (wr *WebArchiveRoutes) Setup(e *echo.Echo) {
	...
	g.GET("/archivo/categorias", wr.pageHandler.ShowCategories, wr.webAuthMW.RequirePermission("archive.read"))
	g.POST("/archivo/categorias", wr.pageHandler.SubmitCreateCategory, wr.webAuthMW.RequirePermission("archive.write"))
	g.POST("/archivo/categorias/:code/editar", wr.pageHandler.SubmitUpdateCategory, wr.webAuthMW.RequirePermission("archive.write"))
	g.POST("/archivo/categorias/:code/desactivar", wr.pageHandler.SubmitDeactivateCategory, wr.webAuthMW.RequirePermission("archive.write"))
