...
func RegisterArchiveRoutes(g *echo.Group, handler *handlers.ArchiveHandler) {
	...
	ag.POST("/categories", handler.CreateCategory, authmw.RequirePermission("archive.write"))
	ag.PATCH("/categories/:code", handler.UpdateCategory, authmw.RequirePermission("archive.write"))
	ag.DELETE("/categories/:code", handler.DeactivateCategory, authmw.RequirePermission("archive.write"))
