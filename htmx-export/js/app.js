...
(function () {
  ...
  function toggleSidebar() {
    ...
    const expanded = sidebar.classList.toggle('sidebar-collapsed');
  ...
  function updateThemeModeButtons(mode) {
    document.querySelectorAll('.theme-mode-light, .theme-mode-dark').forEach((btn) => {
      btn.classList.remove('border-muted-foreground/30', 'bg-card');
    });
    const active = document.querySelector(mode === 'dark' ? '.theme-mode-dark' : '.theme-mode-light');
    active?.classList.add('border-muted-foreground/30', 'bg-card');
  }
  ...
  /**
   * Listener HTMX global para inyectar token de sesión.
   * TEMPORAL: usar sessionStorage hasta implementar cookies HttpOnly en Go.
   * En producción preferir cookie SameSite=Strict enviada automáticamente.
   */
